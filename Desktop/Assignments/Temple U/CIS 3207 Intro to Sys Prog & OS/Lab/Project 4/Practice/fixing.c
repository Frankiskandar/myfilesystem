...
typedef struct directoryEntry{
	...
	unsigned int systemFile : 1;
	unsigned int volumeLabel : 1;
	unsigned int subdir : 1;
	unsigned int archive : 1;
	unsigned int unusedbit1 : 1;
	unsigned int unusedbit2 : 1;
	char emptyOffset[10]; // offset 12-21 are unused. they are not required for simplicity
	short time; //modified time // offset 22-23
	short date; // modified date //offset 24-25
	short startCluster; // offset 26-27
...
int main(){
	...
	while(b){
		...
		if(cmd=='f'){
			...
			// directory creation
			clearInput();
			printf("Enter directory pathname without spaces\n");
			scanf("%s", input);
			my_CreateDirectory(input);
		}
		else if(cmd == 'c'){
			//file creation
			//sometimes it fails to create a file in ROOT
			//adding a number in file can also fail the process
			// dont have time to debug for now
			clearInput();
			printf("Enter file pathname with extension eg. '.txt'\n");
			scanf("%s", input);
			my_CreateFile(input);
		}	
		else if(cmd=='q')
			...
			clearInput();
			printf("Please specify the pathname of the file to open\n");
			scanf("%s", input);
...
int findEmptySector(){
	...
	for(;i<=9688;i++){
		if(fat->next == 0){ //if we find the first available cluster
...
{	
	...
	*ex = '\0';
	...
	fseek(disk, firstByte(RD_START_SECTOR), SEEK_SET);
		...
		//got to file in pathname, find entry and set file 
		else
		{	
			if(strcmp(e, names[j])==0)
			{
